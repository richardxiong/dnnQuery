...
class Seq2SeqModel(object):
  ...
  def __init__(self,
    ...
    """
    subset = 'restaurants'
    self.from_vocab_path = './three_learn/ba_ca_ho_%s/vocab1500.from' % subset
    self.to_vocab_path = './three_learn/ba_ca_ho_%s/vocab150.to' % subset
    ...
    for i in xrange(buckets[-1][0]):  # Last bucket is the biggest one.
      self.tag_inputs.append(tf.placeholder(tf.int32, shape=[None],
                                                name="encoder{0}".format(i)))
    ...
    if forward_only:
      self.outputs, self.losses, self.lasthiddens = ls.model_with_buckets_tag(  ##### 
          self.encoder_inputs, self.tag_inputs, self.decoder_inputs, targets,
  ...
  def step(self, session, encoder_inputs, tag_inputs, decoder_inputs, target_weights,
    ...
    if len(tag_inputs) != encoder_size:
      raise ValueError("Tag length must be equal to the one in bucket,"
                       " %d != %d." % (len(encoder_inputs), encoder_size))
    ...
    for l in xrange(encoder_size):
      ...
      input_feed[self.tag_inputs[l].name] = tag_inputs[l]
    ...
    if not forward_only:
      output_feed = [self.updates[bucket_id],  # Update Op that does SGD.
                     ...
                     self.lasthiddens[bucket_id]]  # Loss for this batch.
    else:
    ...
    if not forward_only:
      return outputs[1], outputs[2], None, outputs[3]###,   Gradient norm, loss, no outputs, last encoder hidden states.
    else:
  ...
  def get_batch(self, data, bucket_id, idx = None):
    ...
    """
    encoder_size, decoder_size = self.buckets[bucket_id]
    encoder_inputs, tag_inputs, decoder_inputs = [], [], []
    ...
    for i in xrange(self.batch_size):
      ...
      tag_inputs.append(list(reversed(tag_input[0:len(encoder_input)] + encoder_pad)))

      # Decoder inputs get an extra "GO" symbol, and are padded then.
      decoder_pad_size = decoder_size - len(decoder_input) - 1
      decoder_inputs.append([data_utils_tag.GO_ID] + decoder_input +
                            [data_utils_tag.PAD_ID] * decoder_pad_size)

    # Now we create batch-major vectors from the data selected above.
    batch_encoder_inputs, batch_tag_inputs, batch_decoder_inputs, batch_weights = [], [], [], []
    ...
    for length_idx in xrange(encoder_size):
      ...
      batch_tag_inputs.append(
          np.array([tag_inputs[batch_idx][length_idx]
                    for batch_idx in xrange(self.batch_size)], dtype=np.int32))
    ...
    for length_idx in xrange(decoder_size):
      ...
      for batch_idx in xrange(self.batch_size):
        ...
        if length_idx == decoder_size - 1 or target == data_utils_tag.PAD_ID:
          batch_weight[batch_idx] = 0.0
      batch_weights.append(batch_weight)
    return batch_encoder_inputs, batch_tag_inputs, batch_decoder_inputs, batch_weights